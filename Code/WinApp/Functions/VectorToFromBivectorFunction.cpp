...
GAVisToolVectorToFromBivectorFunctionEvaluator::GAVisToolVectorToFromBivectorFunctionEvaluator( FuncType funcType )
{
	...
	if( funcType == CONVERT_VECTOR_TO_BIVECTOR )
	{
		strcpy_s( conversionCode, sizeof( conversionCode ),
			...
			"scalar_part( element, no_e1b )*no^e2b + "
			"scalar_part( element, no_e1b )*no^e3b + "
			"scalar_part( element, no_e1b )*no^nib + "

			"scalar_part( element, e1_nob )*e1^nob + "
			"scalar_part( element, e1_e1b )*e1^e1b + "
			"scalar_part( element, e1_e1b )*e1^e2b + "
			"scalar_part( element, e1_e1b )*e1^e3b + "
			"scalar_part( element, e1_e1b )*e1^nib + "

			"scalar_part( element, e2_nob )*e2^nob + "
			"scalar_part( element, e2_e1b )*e2^e1b + "
			"scalar_part( element, e2_e1b )*e2^e2b + "
			"scalar_part( element, e2_e1b )*e2^e3b + "
			"scalar_part( element, e2_e1b )*e2^nib + "

			"scalar_part( element, e3_nob )*e3^nob + "
			"scalar_part( element, e3_e1b )*e3^e1b + "
			"scalar_part( element, e3_e1b )*e3^e2b + "
			"scalar_part( element, e3_e1b )*e3^e3b + "
			"scalar_part( element, e3_e1b )*e3^nib + "

			"scalar_part( element, ni_nob )*ni^nob + "
			"scalar_part( element, ni_e1b )*ni^e1b + "
			"scalar_part( element, ni_e1b )*ni^e2b + "
			"scalar_part( element, ni_e1b )*ni^e3b + "
			"scalar_part( element, ni_e1b )*ni^nib"
		);
	}
	else if( funcType == CONVERT_VECTOR_FROM_BIVECTOR )
	{
		strcpy_s( conversionCode, sizeof( conversionCode ),
			...
			"scalar_part( element, no^e1b )*no_e2b + "
			"scalar_part( element, no^e1b )*no_e3b + "
			"scalar_part( element, no^e1b )*no_nib + "

			"scalar_part( element, e1^nob )*e1_nob + "
			"scalar_part( element, e1^e1b )*e1_e1b + "
			"scalar_part( element, e1^e1b )*e1_e2b + "
			"scalar_part( element, e1^e1b )*e1_e3b + "
			"scalar_part( element, e1^e1b )*e1_nib + "

			"scalar_part( element, e2^nob )*e2_nob + "
			"scalar_part( element, e2^e1b )*e2_e1b + "
			"scalar_part( element, e2^e1b )*e2_e2b + "
			"scalar_part( element, e2^e1b )*e2_e3b + "
			"scalar_part( element, e2^e1b )*e2_nib + "

			"scalar_part( element, e3^nob )*e3_nob + "
			"scalar_part( element, e3^e1b )*e3_e1b + "
			"scalar_part( element, e3^e1b )*e3_e2b + "
			"scalar_part( element, e3^e1b )*e3_e3b + "
			"scalar_part( element, e3^e1b )*e3_nib + "

			"scalar_part( element, ni^nob )*ni_nob + "
			"scalar_part( element, ni^e1b )*ni_e1b + "
			"scalar_part( element, ni^e1b )*ni_e2b + "
			"scalar_part( element, ni^e1b )*ni_e3b + "
			"scalar_part( element, ni^e1b )*ni_nib"
		);
	}
