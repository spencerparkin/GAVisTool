...
Letting $p=e_0+\vec{p}$ and $a=e_0+\vec{a}$, our equation becomes
\begin{equation}
-1 = \vec{p}\cdot\vec{a} = \langle \vec{p}\vec{a}\rangle_0,
\end{equation}
where it is now clear that $a\wedge\overline{a}$ is the solitary point
$e_0-\vec{a}^{-1}$ in homogenized form.  What is perhaps unfortunate is
...
\end{equation}
Clearly $a\wedge\overline{d}$ represents the pair of points
$e_0-\vec{a}^{-1}$ and $e_0-\vec{d}^{-1}$ if we
let $a=e_0+\vec{a}$ and $d=e_0+\vec{d}$.
...
if and only if $p\cdot a=0$ and $p\cdot b=0$, which is impossible unless $a=b$.
Therefore, if $a=b$, we can say that $a+\overline{b}$ is a point.\footnote{Notice that we must be careful here in referring to a
