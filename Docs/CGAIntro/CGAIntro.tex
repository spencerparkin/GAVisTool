...
A note about my efforts in precision in this paper is warrented before we begin.
Where most other authors attempt to avoid pedanticism on this subject, I may
be in danger of entering in upon such a form of writing, because
I do not wish to incur any expense of confusion that I find too often in other writings
on this subject.  So that being said, if the reader can bare with me in these instances, (and I believe
it will be much easier to do so for anyone such as myself who is not always adept
at or tolerant of interpreting the informal statements of some technical writers), then we can move forward.
...
such vectors, we can see that any geometry dually represented by such
a product is homogenized whenever every vector in any factorization of that blade
is homogenized.

We now pose the following question.  What geometric significance does
the scalar $\lambda\in\R$ hold with respect to the geometry dually represented by a blade $B\in\G$
when $\lambda^{-1} B$ is the homogenization of $B$, and $B$ was formulated as a
dual of an outer product of homogenized vectors representative of points?  That is,
for a set of $m$ points $\{x_k\}_{k=1}^m$, we have
\begin{equation*}
B = \pm\left(\bigwedge_{k=1}^m p(x_k)\right)I.
\end{equation*}
After choosing a sign for the dual, (multiply by $-I$ or $+I$), the sign of $\lambda$
has significance as well.  We will answer the question posed here for
$n$-dimensional hyper-spheres and $(n-1)$-dimensional hyper-planes, and then
answer the question in general.  Of course, we know that flat points are not
applicable here as they cannot be written as the dual of an outer product of
round point representatives.
