...
important questions in the model and even consider a functional
...
$\lambda\in\R$ such that $\lambda^{-1}B$ is the homogenization of $B$?
...
by a scalar that is geometrically significant to all involved in the transformation.
...
of flat points preserve handedness.
...
operation,\footnote{We also don't have time to consider the question of how
many possible geometric operations there are, and what they are.  It stands to
reason, however, that every possible blade-valued function of blades taken from $\G$ must produce
a result of geometric significance in relation to the blades given to the function.  Indeed, there
may even become a usefulness somehow of grouping geometries using multivectors are representatives.}
here we will choose one that may be fundamental
to the conformal model.  Given a blade $B\in\G$ dually representative
...
have some sort of geometric significance.

Suppose first that $B$ dually represents a flat geometry.  Then writing
$B=\bigwedge_{k=1}^m\pi_m$ as the outer product of $m\leq n$
vectors $\{\pi_k\}_{k=1}^m$, each homogenized and dually
representative of an $(n-1)$-dimensional hyper-plane, it is possible to see
that we can, without loss of generality, assume\footnote{This assumption
is easy to prove for $n\leq 3$ by purely geometric arguments, but not entirely obvious for $n>3$,
in which cases an algebraic proof may be in order.}
that exactly
$m-1$ of these vectors are representative of a plane containing the point $x$.
It follows that for some integer $k\in[1,m]$, we have
\begin{equation*}
p(x)\cdot B = (-1)^k (p(x)\cdot \pi_k)\bigwedge_{i=1,i\neq k}^m \pi_k.
\end{equation*}
The geometry being dually represented here is now clear, as well as the change in handedness
and soon, the geometric significance of the homogenization scalar $p(x)\cdot\pi_k$.
So what does this scalar represent?  Letting $v\in\V^n$ be a unit-length vector
normal to the plane $\pi_k$, and letting $\alpha\in\R$ be a scalar such that
$x-\alpha n$ is a point on this plane, we find that
\begin{equation*}
p(x)\cdot\pi_k = p(x)\cdot(n+(n\cdot(x-\alpha n))\nvai) = \alpha.
\end{equation*}
And what is $\alpha$?  It's the shortest or orthogonal distance
of $x$ to the plane $\pi_k$!

Suppose now that $B$ dually represents a round geometry.  Then,
if $B$ dually represents an $(m<n)$-dimensional sphere,
then there exists $n-m$ vectors $\{\pi_k\}_{k=1}^{m-n}$
each dually representative of an $(n-1)$-dimensional hyper-plane, such that
$B=\sigma\wedge A$, where $A=\bigwedge_{k=1}^{m-n}\pi_k$ and
$\sigma$ is dually representative of an $n$-dimensional hyper-sphere.
We can then assume, again, without loss of generality, that
exactly $n-m-1$ of the vectors in $\{\pi_k\}_{k=1}^{m-n}$
are representative of planes containing $x$.  Blah...
...
Our first specimen is important geometry.
...
The geometric function dually representative of this surface is given by
\begin{equation*}
