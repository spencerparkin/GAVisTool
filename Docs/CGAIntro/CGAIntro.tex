...
\begin{equation*}
2r(p(c)+\frac{1}{2}r^2\nvai)\wedge(n+(c\cdot n)\nvai) = p(c-rn)\wedge p(c+rn).
\end{equation*}
Here, $n\in\V^n$ is a unit-length vector.
...
vector $n\in\V^n$, we have $rn\wedge\sigma_i\neq 0$ whenever $rn\wedge\sigma_r\neq 0$
since $\sigma_i$ is a scalar multiple of $\sigma_r$ by our inductive hypothesis.
It is then easy to see that $rn\wedge\sigma_i$, if non-zero,
is dually representative of an imaginary sphere of one lower dimension.  Then, if
it can be shown that $rn\wedge\sigma_r=p(rn)\wedge\sigma_r$, then this blade is directly representative
...
by theorem $\eqref{thm_flat_xor_round}$, and that $p(rn)\cdot\sigma=0=rn\cdot\sigma$.  We then have
\begin{align*}
p(rn)\wedge\sigma_r &= p(rn)\wedge(\sigma\wedge\pi)I \\
 &= (p(rn)\cdot(\sigma\wedge\pi))I \\
 &= ((p(rn)\cdot\sigma)\pi - (p(rn)\cdot\pi)\sigma)I \\
 &= ((rn\cdot\sigma)\pi - (rn\cdot\pi)\sigma)I \\
 &= rn\wedge\sigma.
\end{align*}
\end{proof}
