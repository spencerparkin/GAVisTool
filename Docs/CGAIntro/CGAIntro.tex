...
and CLUCalc, both of which can be easily be found on the internet.
...
the unit pseudo-scalar of the geometric algebra generated by $V^n$ and $I$ is
...
on occasion employ in the study of versor transformations.  When each vector
...
that the double spherical reflections produce a dilation in each case.
...
A more thorough treatment of versor transformation in the conformal model can be found in
\cite{dorst07}.
We have only begun to discover what types of transformations we
can do with these basic building blocks.  (There are undoubtedly more combinations to
...
non-homogenized, then what is the geometric significance of the scalar
...
we get an ellipse as illustrated in the following figure.
\begin{figure}[H]
\centering
...
\end{equation*}
where $\alpha=\sqrt{r^2(1-(v\cdot w)^{-2})}$, is the set of all blades representative
