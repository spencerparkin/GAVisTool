...
shown that 2-blades dually representative of circles are indistinguishable
from 2-blades directly representative of point-pairs.  Indeed, we may
reinterpret one as the other, and get a result of geometric significance
to what we originally had.  Specifically, the following identity holds.
\begin{equation*}
p(c-rv)\wedge p(c+rv) = (p(c)-\frac{1}{2}r^2\nvai)\wedge(v+(c\cdot v)\nvai),
\end{equation*}
where $c\in\V^n$ is the center of both the circle and the point-pair, $r\in\R$ is the radius
of both, and $n\in\V^n$ is the unit-length normal of both.
