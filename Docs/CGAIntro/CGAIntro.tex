...
\section{Imaginary Geometry}

How do we build up imaginary rounds?
...
So let's consider all vectors.  A distinguishing characteristic between
points and spheres is simply that vectors representative of the former are null while those
of the latter are not.  Right away this is something we can use to decispher
a given vector.  Recalling that any multiple of $\nvai$ is, strictly speaking, not a point, it
is not hard to see that all other null vectors must be points.  The remaining non-null
vectors are divided between the $n$-dimensional spheres and $(n-1)$-dimensional
hyper-planes.  How might we distinguish between these?

Considering now all 2-blades, an interesting result comes up.  It can be
shown that 2-blades dually representative of imaginary circles are indistinguishable
from 2-blades directly representative of real point-pairs.  Indeed, we may
reinterpret one as the other, and get a geometrically significant result.
Specifically, the following identity holds.
\begin{equation*}
2r(p(c)-\frac{1}{2}r^2\nvai)\wedge(n+(c\cdot n)\nvai) = p(c-rn)\wedge(c+rn),
\end{equation*}
where $c\in\V^n$ is the center of both the circle and the point-pair, $r\in\R$ is the radius
of both, and $n\in\V^n$ is the unit-length normal of both.  Realizing that
normalized translation versors preserve handedness, a good approach to
proving this identity is to rewrite it as
\begin{equation*}
T[2r(p(0)-\frac{1}{2}r^2\nvai)\wedge n]\tilde{T} = T(p(-rn)\wedge p(rn))\tilde{T},
\end{equation*}
where $T=1-\frac{1}{2}c\nvai$.  This identity is easier to prove.

Similarly, real dual circles are indistinguishable from imaginary direct
point-pairs.  Show that here...

What we've now shown is that for a given 2-blade, if we've ruled out all
possibilities except for the circle and point-pair, then we're out of luck.
The blade could be interpreted as either one.

We're now left to consider lines and flat points.  No 2-blade representative
of a flat point can be mistaken as representing a line.  Show why...
...
Suppose first that $B$ dually represents an $(m<n)$-dimensional
hyper-plane.  It then follows by result $\eqref{rslt_intersect_grades}$
...
\end{equation*}
We see
now that the geometry being dually represented here is an
...
$x-\alpha n$ is a point on this hyper-plane, we find that
\begin{equation*}
...
Suppose now that $B$ dually represents an $(m\leq n)$-dimensional
hyper-sphere.  It then follows by result $\eqref{rslt_intersect_grades}$
that...
