...
any point $x\in\V^n$, then we we understand the transformation $VBV^{-1}$,
where $B$ is any blade directly or dually representative of any geometry.
\end{result}
\begin{proof}
...
Considering a flat point $p(x)\wedge\nvai$, we need only show that $\nvai$ is,
up to scale, invarient under any versor transformation.

To show now that the theorem goes through for blades dually representative of geometries,
...
apply to any geometry of the conformal model, no matter the representation!
...
\subsection{Spherical Reflections}
...
Throughout this paper we did not always give the weight of a
conformal geometry much thought.  If we were to go back and examine
what weights come out of certain operations between homogenized geometries,
...
Indeed, this introduction has only just scratched the surface of
...
with specific features, we might as well choose a geometric aglebra,
