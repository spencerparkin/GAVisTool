...
\end{proof}
We're now ready to prove a theorem that we'll use to proof something very interesting
...
$f(B)=VBV^{-1}$ is an outermorphism.  It then follows that if we understand
how $V$ transforms a conformal point, then we
also understand how $V$ transforms any directly represented conformal geometry by virtue of
choosing any factorization for that geometry in terms of conformal points!  This is an
extraordinary property of the conformal model.

Of course, there are a few more details to work out here.  As we now know,
all but the flat point geometry can be expressed directly as an outer product
of round points.  So we'll want to consider the action of $V$ on $\nvai$.
We'll also want to consider the action of $V$ on a blade dually representive
of a conformal geometry.
...
Two well chosen reflections about two planes make a rotation.

\subsection{Translations}

Two well chosen reflections about two planes make a translation.
...
Throughout this paper we did not give the weight of a
conformal geometry much thought.  If we were to go back and examine
what weights come out of operations between homogenized geometries,
then we would probably discover that those weights have interesting
and valuable geometric significance to the operations at hand.
