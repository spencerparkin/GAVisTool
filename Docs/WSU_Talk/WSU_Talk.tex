...
\alert{Attitude} matters, \alert{position} does not.
...
\begin{frame}
\frametitle{Surprise!}
\alert{Blades} representative of geometry are also \alert{versors} representative of transformations!
\begin{itemize}
\item \alert{Planar reflections} are represented by a dual planes!
\item \alert{Spherical reflections} are represented by dual spheres!
\item \alert{Translations} \& \alert{Rotations} are two successive planar reflections in well chosen planes.
\item \alert{Dilations} \& \alert{Transversions} are two successive spherical reflections in well chosen spheres.
\end{itemize}
\alert{Corollary}: We can use versors to transform transformations!
