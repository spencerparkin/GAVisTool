...
linearly independent set, then so is $\{f(e_i)\}_{i=1}^n$.
...
and is the condition upon which $f^{-1}$ exists.  Clearly, if there
exist distinct vectors $x,y\in\V^n$ such that $f(x)=f(y)$, then no
function purporting to be $f^{-1}$ is well defined.
...
prove the uniqueness of $f^{-1}$.  Suppose the function $g$ and $h$ are
...
would now hope is that of having a means of finding $f^{-1}$.
